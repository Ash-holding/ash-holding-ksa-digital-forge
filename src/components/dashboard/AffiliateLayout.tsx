...
import {
  LayoutDashboard, Link2, Megaphone, Users, Wallet, Bell,
  Menu, X, LogOut, Sparkles, BadgeDollarSign, User,
} from "lucide-react";
...
const ITEMS = [
  { to: "/affiliate", label: "نظرة عامة", icon: LayoutDashboard, exact: true },
  { to: "/affiliate/links", label: "روابطي", icon: Link2 },
  { to: "/affiliate/campaigns", label: "الحملات", icon: Sparkles },
  { to: "/affiliate/marketing", label: "المركز التسويقي", icon: Megaphone },
  { to: "/affiliate/customers", label: "العملاء المُحالون", icon: Users },
  { to: "/affiliate/commissions", label: "العمولات", icon: BadgeDollarSign },
  { to: "/affiliate/wallet", label: "المحفظة والسحب", icon: Wallet },
  { to: "/affiliate/notifications", label: "الإشعارات", icon: Bell },
  { to: "/affiliate/profile", label: "الملف الشخصي", icon: User },
] as const;

const BOTTOM = [
  { to: "/affiliate", label: "الرئيسية", icon: LayoutDashboard, exact: true },
  { to: "/affiliate/links", label: "الروابط", icon: Link2 },
  { to: "/affiliate/marketing", label: "التسويق", icon: Megaphone },
  { to: "/affiliate/commissions", label: "العمولات", icon: BadgeDollarSign },
  { to: "/affiliate/profile", label: "حسابي", icon: User },
] as const;

export function AffiliateLayout({ children }: { children: ReactNode }) {
    ...
    <div dir="rtl" className="min-h-screen bg-background text-foreground flex">
      <aside className="hidden lg:flex w-60 shrink-0 border-l border-border bg-card/50 flex-col">
        <div className="h-16 flex items-center gap-2 px-5 border-b border-border">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 shadow-glow">
            <Sparkles className="h-4 w-4 text-white" />
          </div>
          <div className="leading-tight">
            <div className="text-sm font-bold">شركاء ASH</div>
            <div className="text-[10px] text-muted-foreground">بوابة المسوّق</div>
          </div>
        </div>
                  ...
                  return (
                    <Link key={it.to} to={it.to} onClick={() => setOpen(false)}
                      className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium",
                        active ? "bg-amber-500/10 text-amber-500" : "text-muted-foreground hover:bg-muted/50")}
                    >
                      <it.icon className="h-4 w-4" />
                      {it.label}
                    </Link>
        ...
        <header className="h-16 border-b border-border bg-background/70 backdrop-blur-md sticky top-0 z-30">
          <div className="h-full px-4 md:px-6 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs text-muted-foreground">أهلاً بشريكنا</div>
              <div className="text-sm font-bold truncate">{user?.name}</div>
            </div>
            <Link to="/affiliate/notifications" className="grid h-9 w-9 place-items-center rounded-lg border border-border">
